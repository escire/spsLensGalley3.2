...
/**
 * @file plugins/generic/spsLens/SPSLensPlugin.inc.php
 *
 ...
 *
 * @class SPSLensPlugin
 * @ingroup plugins_generic_spsLens
 *
 * @brief Class for SPSLens plugin
 */

import('lib.pkp.classes.plugins.GenericPlugin');

class SPSLensPlugin extends GenericPlugin {
	/**
	...
	function getDisplayName() {
		return __('plugins.generic.spsLens.displayName');
	}
	...
	function getDescription() {
		return __('plugins.generic.spsLens.description');
	}
	...
	function articleDownloadCallback($hookName, $args) {
		...
		if ($galley && in_array($galley->getFileType(), array('application/xml', 'text/xml')) && $galley->getFileId() == $fileId) {
			if (!HookRegistry::call('SPSLensPlugin::articleDownload', array($article,  &$galley, &$fileId))) {
				...
				echo $xmlContents;
				$returner = true;
				HookRegistry::call('SPSLensPlugin::articleDownloadFinished', array(&$returner));
			}
			return true;
		}
